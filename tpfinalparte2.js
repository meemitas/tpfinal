...
//https://www.youtube.com/watch?v=hLeCIQAm0Y8
...
class Trineo {
  ...
  dibujar() {
    ...
    if (imgs["trineo"]) {
      ...
      imageMode(CORNER);
    } else if (imgs["trineo_placeholder"]) {
      imageMode(CENTER);
  ...
  chocaCon(gigante) {
    ...
    // ===== INICIO DE LA CORRECCIÓN DE HITBOX =====
    // Se aumenta el margen para hacer la hitbox más justa y pequeña
    let margen = 25; 
    // Se aplica el margen a ambos objetos (Trineo y Gigante)
    ...
    // ===== FIN DE LA CORRECCIÓN DE HITBOX =====
    return false;
  }
}
...
class Gigante {
  ...
  dibujar() {
    ...
    if (imgs["gigante"]) {
      ...
      imageMode(CORNER);
    } else if (imgs["gigante_placeholder"]) {
      imageMode(CENTER);
